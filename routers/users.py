...
@router.post("/register", response_description="Register user", response_model=UserModel)
async def register(request: Request, newUser: LoginModel = Body(...)) -> UserModel:
    ...
    existing_user = await users.find_one({"username": newUser_dict["username"]})
    if existing_user is not None:
        raise HTTPException(
            status_code=409,
            detail=f"User with username {newUser_dict['username']} already exists",
        )

    # Insert new user
    new_user = await users.insert_one(newUser_dict)
